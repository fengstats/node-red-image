...
module.exports = {
  ...
  // 内网部署时直接去除登录密码,md,把自己防住了,不要告诉用户 Node-RED 地址就好
  // adminAuth: {
  //   type: 'credentials',
  //   users: [
  //     {
  ...
  //       username: 'lianhe',
  //       // 123qwe123
  //       password: '$2a$08$4ziI6aKtypM5RGbBbW.qtuCMulEo33oJxFjidL3WmQwfRuDgtnRni',
  //       permissions: '*'
  //     }
  //   ]
  // },
  ...
  logging: {
    ...
    logstash: {
      ...
      metrics: false,
      /*
      启动服务时会调用此函数,当有符合该level的日志时会调用函数内部返回的函数&传递日志信息: handler()(msgObj)
      示例如下,但是有些返回有有些情况可能没有,没有就是undefined
        {
          level: 30,
          id: 'f32a170dd965e84b',
          type: 'function',
          ...
          msg: '呜呜呜',
          z: '59fbae0f90b17e72',
          path: '59fbae0f90b17e72',
          timestamp: 1650276963612
        }
       */
      handler: function (conf) {
        ...
        log4js.configure({
          appenders: {
            info: {
              type: 'file',
              filename: path.resolve(__dirname, './logs/node-red.log'),
              // 日志达到10兆字节(MB)备份(滚动)一次旧日志,单位: 字节(B)
        ...
        return function (msgObj) {
          ...
          // console.log(msgObj)
          try {
            let { level, type, msg, name } = msgObj
            let log = ''
            type && (log += `${type} - `)
            name && (log += `${name} - `)
